...
public class SpreadSheet extends JFrame {
    ...
    public String evaluateToken(String token, AtomicInteger depth) {
        if (token.length() < 2) {
            return null;
        }

        if (token.contains("(") || token.contains(")")) {
            return evaluateComplexExpressions(token, depth);
        } else {
            return evaluateSimpleExpressions(token, depth);
        }
    }

    private String evaluateComplexExpressions(String token, AtomicInteger depth) {
    ...
    private String evaluateSimpleExpressions(String token, AtomicInteger depth) {
        if (token.charAt(0) < 'A' &&  token.charAt(0) >= (char) ('A' + maxCols)) {
            return null;
        }
    ...
    public String parseFormula(StringTokenizer tokens, AtomicInteger depth) 
            throws NumberFormatException {
        if (tokens.hasMoreTokens()) {
            String tok1 = tokens.nextToken();
            String tok = evaluateToken(tok1, depth);
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if (tok2.equals("+")) {
                    ...
                    StringTokenizer tokens1 = new StringTokenizer(tok, "=+*/-:", true);
                    StringTokenizer tokens3 = new StringTokenizer(tok3, "=+*/-:", true);
                    if (tok1.startsWith("AVG")) {
                        depth.incrementAndGet();
                        String result1 = parseFormula(tokens1, depth);
                        String result2 = parseFormula(tokens3, depth);
                        Double sum = Double.parseDouble(add(result1, result2));
                        tok = Double.toString(sum / depth.get());
                    }
    ...
    public void evaluate(int r, int c, AtomicInteger depth) {
        String formula = cells[r][c].formula;
        if (formula.length() > 0 && formula.charAt(0) == '=') {
            try {
                if (depth.get() <= maxRows * maxCols) {
    ...
    public void evaluate() {
        ...
        for (int r = 0; r < maxRows; r++) {
            for (int c = 0; c < maxCols; c++) {
                if (!cells[r][c].valid) {
                    evaluate(r, c, new AtomicInteger(1));
                }
            }
        }
